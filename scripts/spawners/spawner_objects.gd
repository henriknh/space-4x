...
var prefab_asteroid = preload('res://prefabs/asteroid.tscn')

var asteroid_base_distance_to_sun = 200
var asteroid_extra_distance = 1000

func create(target: Node, star_system_idx: int, star_system_size: int) -> void:
	for idx in range(WorldGenerator.rng.randi_range(25, 50)):
		var angle = WorldGenerator.rng.randf() * 2 * PI
		var distance = asteroid_base_distance_to_sun + WorldGenerator.rng.randf() * (star_system_size + asteroid_extra_distance)
